...
namespace Passave
{
    ...
    {
        public static byte[,] secureKey = new byte[4, 4];
        ...
        private void SecureKeyButton_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ofd.Title = "Open secure key file";
                ofd.Filter = "Passave Secure Key (*.psvkey)|*.psvkey";

                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    using (StreamReader sr = new StreamReader(ofd.FileName))
                    {
                        secureKey = MainForm.StringToByteBlock(sr.ReadLine());
                        MainForm.isSecureKey = true;
